...
def summarize(casepath, sum_len, Y, phrase_vector_len=100):
    ...
    model = Word2Vec(data, size=500, window=5, min_count=1)
    ...
    sim2 = np.zeros((len(text),len(text)))
    for i, sentence1 in enumerate(data): 
        for j, sentence2 in enumerate(data):
            if i == j:
                sim2[i][j] = 0
            else:
                sim_score = model.n_similarity(sentence1, sentence2)
